...
const SnacksMunchies = () => {
  const [mydata, setMydata] = useState([]);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  // 🔹 Load Snacks & Munchies products
  const loadData = async () => {
    try {
      const api = `${BackendURL}/blinkitproduct/snacks`;
      const res = await axios.get(api);
      setMydata(res.data);
    } catch (error) {
      console.log("Snacks API Error 👉", error);
    }
  };
  ...
  return (
    <div  >
      <h2 className="category-title">Snacks & Munchies</h2>
